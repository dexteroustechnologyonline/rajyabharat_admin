...
import { uniTagDelete } from "../../../redux/universaltag/universalTagSlice";
...
const ListUniversalTag = () => {
  ...
  const deleteClick = async (e) => {
    ...
    const deletclick = await dispatch(uniTagDelete(id));
    if (deletclick.payload.success) {
      setModal(false);
      messageApi.open({
        type: "success",
        content: "Universal Tag deleted  successfully",
      });
    }
  };

  useEffect(() => {
    let filter_trans = universalTags;
    if (search !== "") {
      filter_trans = filter_trans.filter((transaction) => {
        return transaction.name.toLowerCase().match(search.toLowerCase());
      });
    }
  ...
  }, [search, end_date, start_date, universalTags]);
      ...
      <div className="main_fragnent_container p-2">
        <div className="profile_container_header">
          <div className="profile_co_header_left">
            <span>Search Universal Tag</span>
            <span
              style={{
                visibility: "hidden",
              }}
                      ...
                      <StyledTableRow key={index}>
                        ...
                        >
                          <span
                            ...
                            onClick={() => {
                              navigate(
                                `/admin/update-universal-tag/${data.slugUrl}`
                              );
                            }}
                          >
                            {data.name}
                        ...
                        >
                          ...
                          >
                            <Link
                              to={`/admin/update-universal-tag/${data.slugUrl}`}
                            >
...
export default ListUniversalTag;
