...
import {
  ...
  universaltagUpdate,
  updateUniTagThumbnail,
  validateSlugUrl,
} from "../../../redux/universaltag/universalTagSlice";

const UpdateUniversalTag = () => {
  const dispatch = useDispatch();
  const params = useParams();

  const { isUniTagthumbLoading, uniTagthumb, universalTags } = useSelector(
  ...
  const [universalTagId, setUniversalTagId] = useState("");
  const [universalTagFiltered, setUniversalTagFiltered] = useState([]);

  useEffect(() => {
    const unitagUrl = params.unitagurl;
    if (universalTags.length > 0) {
      const updateunitag = universalTags.find(
        (unitag) => unitag.slugUrl === unitagUrl
      );

      if (updateunitag) {
        setUniversalTagId(updateunitag._id);
        const categoryfilter = universalTags.filter(
          (category) => category._id !== updateunitag._id
        );
        setUniversalTagFiltered(categoryfilter);
        setUniversalTag(updateunitag.name);
        setCheckSlugUrl(updateunitag.slugUrl);
        dispatch(updateUniTagThumbnail(updateunitag.thumbnail));
      }
    }
  }, [params.unitagurl, universalTags]);
  ...
  const verifyslugurl = async (e) => {
    ...
    let slug = value
      ...
      .replace("--", "-");
    if (slug != "") {
      const slugverify = universalTagFiltered.filter(
        (unitag) => unitag.slugUrl === slug
      );
      if (slugverify) {
        setCheckSlugUrl("");
      } else {
  ...
  const handlesubmit = async (e) => {
    ...
    if (uniTagthumb !== "" && checkslugUrl !== "") {
      const formData = {
        ...
        uitagid: universalTagId,
      };
      const data = await dispatch(universaltagUpdate(formData));

      if (data.payload.success) {
        messageApi.open({
          type: "success",
          content: "Universal Tag updated successfully",
        });
        setbtnLoading(false);
        setbtnPress(false);
      } else {
...
export default UpdateUniversalTag;
