...
const AddNews = () => {
  ...
  const [newsContentTelugu, setNewsContentTelugu] = useState("");
  const [newsContentEnglish, setNewsContentEnglish] = useState("");

  const [newsShortName, setNewsShortName] = useState("");
  const [newsShortNameTelugu, setNewsShortNameTelugu] = useState("");
  const [newsShortNameEnglish, setNewsShortNameEnglish] = useState("");
  ...
  const verifyslugurl = async (e) => {
    ...
    if (value.trim() === "") {
      setNewsShortNameTelugu("");
      setNewsShortNameEnglish("");
      return;
    }
    let slug = value
      .trim()
    ...
    try {
      // Translate to English and Telugu using LibreTranslate
      ...
      const enTranslation = await translateText(value, "en");
      setNewsShortNameEnglish(enTranslation);
      const teTranslation = await translateText(value, "te");
      setNewsShortNameTelugu(teTranslation);
    } catch (error) {
  ...
  const handlesubmit = async (e) => {
    ...
    if (
      ...
      const data = await dispatch(newsPost(formData));
